...
=== Setup infrastructure for required environment(Rails 4.0 + Mongodb)
...
Create or change the production group to include mongoid
#Ensure the mongoid gem is defined in your Gemfile 
...
=== Create service (Mongodb)

    $ app42 createService
    
DB Configure for Production environment (application_root_dir/config/mongoid.yml) 
